...
ave_flow_list=( 10.0)
static_dia=40.0
reduced_speed_list=(0.0  1.0   2.0   3.0  6.0 )
    ...
    do
        ...
        do
            ...
            do
                ...
                # nohup time python snapshot.py $rho $ave_flow $static_dia $reduced_speed  $rotational_diffusion > ${name}.log 2>&1 &
                # nohup time python phai6_area.py $rho $ave_flow $static_dia $reduced_speed  $rotational_diffusion > ${name}.log 2>&1 &
                # nohup time python phai6.py $rho $ave_flow $static_dia $reduced_speed  $rotational_diffusion > ${name}.log 2>&1 &
                nohup time python phai6_scatter.py $rho $ave_flow $static_dia $reduced_speed  $rotational_diffusion > ${name}.log 2>&1 &
