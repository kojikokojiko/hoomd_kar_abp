...
for ave_flow in ave_flow_list:
    for active_force in acive_force_list:
        ...
        temp_dir="{0}_{1}_{2}_{3}_{4}".format(rho,ave_flow,static_dia,active_force,D)
...
for dir in files_dir:
    ...
    plt.hist(data,bins=100,alpha=0.5,density=True,label=r'$v_0={0}$'.format(v0))
    # plt.ylim(0.0 ,18)
    plt.xlim(0.0,1.0)
    plt.yscale("log")
    plt.legend()
    ...
    plt.title(r'Probability Density ($U={0}$)'.format(U))
    # plt.ylabel(r"$f(\|\phi_6\|)$",fontsize=15)
    plt.xlabel(r'$\|\phi_6\|$',fontsize=15)
    # sns.kdeplot(data=data)
output_path="./phi6_hist_{0}.png".format(U)
